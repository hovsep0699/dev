...
import SendContainerHandler from './SendContainerHandler';

class SignContainerProcessHandler extends SignHandlerBase {
  sign(request) {
    super.sign(request);
    if (this.canHandle(request)) {
      ...
      const signProcessChain = SignService.getSignContainerProcessChain();
  ...
  handleSignProcessEnding(request) {
    ...
    if (request.documents.length === request.signedDocumentIds.size) {
      const sendingContainer = new SendContainerHandler();
      this.setNext(sendingContainer);
    }
  }
}

export default SignContainerProcessHandler;
