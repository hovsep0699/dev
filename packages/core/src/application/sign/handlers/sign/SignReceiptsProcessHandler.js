...
class SignReceiptsProcessHandler extends SignHandlerBase {
  sign(request) {
    super.sign(request);
    if (this.canHandle(request)) {
      SignService.events.notify('startSignProcess', request); //TODO change events to startSignReceiptsProcess
      ...
      const signProcessChain = SignService.getSignAndSendReceiptsProcessChain();
  ...
  handleSignProcessEnding(request) {
    ...
    if (request.isAutoMode) {
      const receiptsHandler = new ReceiptsHandler();
      const signHandler = new SignReceiptsProcessHandler();
      this.setNext(receiptsHandler).setNext(signHandler);
    }
  }
}

export default SignReceiptsProcessHandler;
